...
namespace _Morrigan.Scripts.ActionSystem
{
    ...
    public class ActionListEditor : Editor
    {
        ...
        private static void PopulateActions()
        {
            ...
            foreach (var type in Assembly.GetAssembly(typeof(Action)).GetTypes().Where(t => t.IsClass && !t.IsAbstract && t.IsSubclassOf(typeof(Action))))
            {
        ...
        {
            var array = serializedObject.FindProperty("actions");
            var newArray = new List<Action>(array.arraySize);
            for (var i = 0; i < array.arraySize; i++)
            {
                var action = array.GetArrayElementAtIndex(i);
            ...
            serializedObject.FindProperty("actions").SetUnderlyingValue(newArray);
        }
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (_toDelete != -1)
            {
                array.DeleteArrayElementAtIndex(_toDelete);
                _toDelete = -1;
            }
        ...
        private void DrawRunNowButton()
        {
            ...
            if (GUILayout.Button("Run ActionList now"))
            {
                (target as ActionList)?.Run();
            }
        ...
        {
            var delButtonWidth = GUI.skin.button.CalcSize(new GUIContent(EditorGUIUtility.IconContent("d_TreeEditor.Trash"))).x;
        ...
        private static void DrawMoveActionButtons(Rect rect, int i, SerializedProperty array)
        {
            if (i > 0)
            {
                var moveUpRect = new Rect(rect.width - _actionContainer.padding.left, rect.y - _actionContainer.padding.top + 4, _moveUpSize.x, _moveUpSize.y);
            ...
            var moveDownRect = new Rect(rect.width - _actionContainer.padding.left, rect.y + _actionContainer.padding.bottom + _moveUpSize.y, _moveDownSize.x, _moveDownSize.y);
        ...
        private void DrawActionEditorFields(Action action)
        {
            ...
            {
                EditorUtility.SetDirty(target.GetComponent<ActionList>());
        ...
        private void AddActionByTitle(string title)
        {
            ...
            var array = serializedObject.FindProperty("actions");
            array.arraySize++;
            var newAction = array.GetArrayElementAtIndex(array.arraySize - 1);
